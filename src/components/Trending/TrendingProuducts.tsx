...
export default function TrendingProducts() {
  ...
  if (loading)
    return (
      <>
        <p className="text-center py-12 text-purple-600 font-semibold">
      ...
      <Carousel opts={{ loop: true }}>
        <CarouselContent className="gap-6 flex justify-start items-start">
          {products.map((product, prodIndex) => (
            <CarouselItem
              key={prodIndex}
              className="basis-auto flex flex-col items-center gap-2"
            >
              <Dialog>
                ...
                <DialogContent className="max-w-md p-4 flex flex-col items-center gap-4">
                  <Image
                    ...
                    height={300}
                  />
                  <p className="font-semibold text-purple-600 text-center">
                    {product.title}
                  </p>
              ...
              <div className="text-center mt-2">
                <p className="font-semibold text-purple-600 truncate max-w-[250px]">
                  {product.title}
                </p>
                <p className="text-lg font-bold text-pink-500">
                  {product.price} EGP
